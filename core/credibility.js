...
function isAiModelRegistryOrProjectSource(hostname, pathname) {
    if ([
        "huggingface.co",
        "github.com",
        "modelscope.cn",
...
function assessDomainCredibility(url) {
    try {
        ...
        if (isAiModelRegistryOrProjectSource(hostname, pathname)) {
            type = "AI model registry/project source";
            credibility = "high";
            signals.push("known AI model registry, code host, research, or vendor source");
        }
